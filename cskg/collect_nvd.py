    ...
    try:
        ...
        for config in nodes:
            for cpe in config.get("cpeMatch", []):
                ...
                cpe_node = Node("CPE", name=cpe_uri, source="NVD", uri=f"http://example.org/cpe#{cpe_uri}")
                graph.merge(cpe_node, "CPE", "name")
                graph.merge(Relationship(cve_node, "affects", cpe_node))
                parsed = parse_cpe(cpe_uri)

                vendor_node = Node("Vendor", name=parsed["vendor"], source="NVD", uri=f"http://example.org/vendor/{parsed['vendor']}")
                product_node = Node("Product", name=parsed["product"], source="NVD", uri=f"http://example.org/product/{parsed['product']}")
                version_node = Node("Version", name=parsed["version"], source="NVD", uri=f"http://example.org/version/{parsed['version']}")
    ...
    try:
        for ref in item["cve"].get("references", []):
            ...
            if "CAPEC-" in url:
                ...
                capec_node = Node("CAPEC", name=capec_id, source="NVD", uri=f"https://capec.mitre.org/data/definitions/{capec_id.replace('CAPEC-', '')}.html")
                graph.merge(capec_node, "CAPEC", "name")
                graph.merge(Relationship(cve_node, "hasCapec", capec_node))

                rdf_capec = URIRef(capec_node["uri"])
